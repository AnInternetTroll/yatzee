...
export class Game {
	...
	constructor({
		...
		if (
			...
			this.objectives = objectives.map(
			...
			this.objectives = [
				...
				new Objective({
					name: "checkThreeRepeats",
					display: "3 like",
				}),
				new Objective({
					name: "checkFourRepeats",
					display: "4 like",
				}),
				new Objective({
					name: "smallStraight",
	...
	#duplicatesCount(dice) {
		...
		for (const arr of dice) {
			counts[arr.value] = counts[arr.value]
				? counts[arr.value] + 1
				: 1;
		}
	...
	checkThreeRepeats() {
		...
		const repeats = Object.entries(duplicateCount).find(
			(dice) => dice[1] >= 3
		);
	...
	checkAPair() {
		const frequencyTable = this.#duplicatesCount(this.dice);

		// [ ["1", 2], ["2", 1] ]
		const entries = Object.entries(frequencyTable);
	
		// Sort by the dice value
		// Not how many dice of that value there are
		entries.sort((a, b) => parseInt(b[0]) - parseInt(a[0]));

		for (const entry of entries) {
			if (entry[1] >= 2) return parseInt(entry[0]) * 2;
		}
	}
	checkTwoPairs() {
		const frequencyTable = this.#duplicatesCount(this.dice);
		...
		for (const entry of entries) {
			if (entry[1] >= 2) {
				output += parseInt(entry[0]) * 2;
				count++;
			}
		}
		console.log("Count: ", count)

		return count === 2 ? output : 0;
	}

	house() {
		const frequencyTable = this.#duplicatesCount(this.dice);
		const values = Object.values(frequencyTable);

		if (values.includes(2) && values.includes(3))
			return this.#addUpAllDice();
	}
	chance() {
		return this.#addUpAllDice();
	}
